...
class RealLuminairePlacer:
    ...
    def place_all(self, plan: ParsedPlan, classified: ClassifiedPlan,
        ...
        if _guard_polys:
            ...
            if clipped > 0:
                log.info(f"Boundary guard: removed {clipped} lights outside floor-plan walls")
                result.corrections.append(
                    f"Boundary guard removed {clipped} lights outside floor-plan walls")

        # ── Final placement summary ───────────────────────────────────────────
        type_counts = Counter(p.lumi_type for p in result.placed)
        total_w     = sum(p.wattage for p in result.placed)
        log.info("=" * 54)
        log.info(f"PLACEMENT COMPLETE: {len(result.placed)} luminaires | "
                 f"{total_w:.0f} W total")
        for t, n in sorted(type_counts.items()):
            log.info(f"  Type {t}: {n} luminaires")
        log.info("=" * 54)
        for corr in result.corrections:
            log.info(f"  Note: {corr}")
    ...
    def _place_sales(self, zone: ZoneResult, plan: ParsedPlan,
        ...
        log.info(f"Sales floor: {zone.area_m2:.0f} m²  |  raw shelf objects: {len(raw_objs)}")

        if not raw_objs:
            log.warning("No shelf objects found — falling back to area grid")
            return self._place_grid_default(zone, excl, INTER_LUMI_MM, ox, oy)
        ...
        if envelope is not None:
            ...
            if _in_env:
                raw_objs = _in_env
            log.info(f"  After envelope filter: {len(raw_objs)} shelf objects")

        _before_mad = len(raw_objs)
        shelf_objs = _mad_filter(raw_objs)
        if len(shelf_objs) != _before_mad:
            log.info(f"  MAD outlier filter: {_before_mad} → {len(shelf_objs)} "
                     f"(removed {_before_mad - len(shelf_objs)} outliers)")

        _sf_buf = zone.polygon.buffer(300)
        _sf_shelves = [f for f in shelf_objs if _sf_buf.covers(Point(*f.position))]
        if _sf_shelves:
            if len(_sf_shelves) != len(shelf_objs):
                log.info(f"  Zone boundary filter: {len(shelf_objs)} → {len(_sf_shelves)} "
                         f"(removed {len(shelf_objs) - len(_sf_shelves)} outside sales zone)")
            shelf_objs = _sf_shelves

        log.info(f"  Shelf objects used for placement: {len(shelf_objs)}")
        ...
        log.info(f"  Candidates from shelf rows: {len(candidates)}")

        if not candidates:
            log.warning("No shelf-row candidates — falling back to area grid")
            ...
            log.info(f"  Area-grid fallback: {len(candidates)} candidates")
        ...
        if _shelf_excl_union is not None:
            ...
            if _removed > 0:
                log.info(f"  Shelf-body exclusion: removed {_removed} on-shelf candidates "
                         f"({len(candidates)} remain)")
            else:
                log.info(f"  Shelf-body exclusion: all {len(candidates)} candidates in aisles ✓")
        ...
        # Wall relation summary
        _aw_count = sum(1 for r in wall_rels if r == 'exterior_wall')
        _a_count  = len(wall_rels) - _aw_count
        log.info(f"  Wall relation: {_aw_count} AW (exterior-wall) | {_a_count} A (interior)")

        # Column objects for feature extraction
        col_objs = [f for f in plan.furniture if f.inferred_type == 'column']
        if col_objs:
            log.info(f"  Columns in zone: {len(col_objs)}")

        # ── 4. Place A-lights and AW-lights via ML (with rule fallback) ───
        visited: set = set()
        placed:  list = []
        use_ml = _load_placer_model() is not None
        log.info(f"  ML model: {'active' if use_ml else 'not found — using rules only'}")
        ...
        if not placed:
            log.warning("No A/AW lights placed — check candidate generation")
            return []

        _type_counts = Counter(p.lumi_type for p in placed)
        log.info(f"  A/AW placed: A={_type_counts.get('A',0)}  AW={_type_counts.get('AW',0)}")
